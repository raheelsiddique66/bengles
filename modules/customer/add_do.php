...
if(isset($_POST["customer_add"])){
	...
	if($err==""){
		$sql="INSERT INTO customer (customer_name, phone, address, customer_name_urdu, balance, machine_id) VALUES ('".slash($customer_name)."', '".slash($phone)."', '".slash($address)."', '".slash($customer_name_urdu)."', '".slash($balance)."', '".slash($machine_id)."')";
		doquery($sql,$dblink);
