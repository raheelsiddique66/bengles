...
if(isset($_POST["action"])){
	...
	switch($_POST["action"]){
		...
		case "get_vendor_delivery":
			$id = slash( $_POST[ "id" ] );
			$rs = doquery( "select a.*, b.name from vendor_delivery a left join labour b on a.labour_id = b.id where a.id='".$id."'", $dblink );
			if( numrows( $rs ) > 0 ) {
				$r = dofetch( $rs );
				...
				$vendor_delivery[ "vendor_delivery_items" ] = $vendor_delivery_items;
			}
			$response = $vendor_delivery;
		break;
		case "save_vendor_delivery":
			$err = array();
			$vendor_delivery = json_decode( $_POST[ "vendor_delivery" ] );
			if( empty( $vendor_delivery->date ) || empty( $vendor_delivery->vendor_id ) ) {
			...
			if( count( $err ) == 0 ) {
				if( !empty( $vendor_delivery->id ) ) {
					doquery( "update vendor_delivery set `date`='".slash(date_dbconvert($vendor_delivery->date))."', `gatepass_id`='".slash($vendor_delivery->gatepass_id)."', `vendor_id`='".slash($vendor_delivery->vendor_id)."', `claim`='".slash($vendor_delivery->claim)."', `labour_id`='".slash($vendor_delivery->labour_id)."' where id='".$vendor_delivery->id."'", $dblink );
					$vendor_delivery_id = $vendor_delivery->id;
				}
				else {
					doquery( "insert into vendor_delivery (date, vendor_id, claim, labour_id, gatepass_id) VALUES ('".slash(date_dbconvert($vendor_delivery->date))."', '".slash($vendor_delivery->vendor_id)."', '".slash($vendor_delivery->claim)."', '".slash($vendor_delivery->labour_id)."', '".slash($vendor_delivery->gatepass_id)."')", $dblink );
