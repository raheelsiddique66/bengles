...
if( numrows( $rs ) > 0 ) {
	...
	while( $r = dofetch( $rs ) ) {
        ...
        $sql="select sum(amount) as amount, sum(discount) as discount, sum(claim) as claim from vendor_payment where vendor_id = '".$r[ "vendorid" ]."'".(!empty($machine_id)?" and machine_id = '".$machine_id."'":"")." and datetime_added>='".date_dbconvert($date_from)." 00:00:00' and datetime_added<='".date_dbconvert($date_to)." 23:59:59'";
