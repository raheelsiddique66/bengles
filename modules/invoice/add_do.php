...
if(isset($_POST["invoice_add"])){
	...
	if($err==""){
		$sql="INSERT INTO invoice (customer_id, datetime_added, date_from, date_to, notes) VALUES ('".slash($customer_id)."','".slash(datetime_dbconvert($datetime_added))."','".slash(date_dbconvert($date_from))."','".slash(date_dbconvert($date_to))."','".slash($notes)."')";
		doquery($sql,$dblink);
