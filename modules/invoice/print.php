...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <table width="100%" cellpadding="0" cellspacing="0">
            ...
            <tbody>
                <?php
                $sql = "select date as datetime_added, gatepass_id, title_urdu as title,  quantity, unit_price, unit_price*quantity as debit, 0 as credit, 0 as discount from delivery a left join delivery_items b on a.id = b.delivery_id left join color c on b.color_id = c.id where customer_id = '".$invoice[ "customer_id" ]."'".($invoice["machine_id"]>0?" and b.machine_id='".$invoice["machine_id"]."'":"")." and date>='".date('Y-m-d',strtotime($invoice["date_from"]))."' and date<='".date('Y-m-d',strtotime($invoice["date_to"]))." 23:59:59' group by color_id,design_id union select datetime_added as datetime_added, '', title, 0, 0, 0, amount as credit, discount as discount from customer_payment a left join account c on a.account_id = c.id where customer_id = '".$invoice[ "customer_id" ]."'".($invoice["machine_id"]>0?" and a.machine_id='".$invoice["machine_id"]."'":"")." and datetime_added>='".date('Y-m-d',strtotime($invoice["date_from"]))."' and datetime_added<'".date('Y-m-d',strtotime($invoice["date_to"]))." 23:59:59' order by datetime_added";
                //echo $sql;die;
