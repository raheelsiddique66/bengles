...
if(isset($_POST["customer_payment_edit"])){
	...
	if($err==""){
		$sql="Update customer_payment set `customer_id`='".slash($customer_id)."',`machine_id`='".slash($machine_id)."',`datetime_added`='".slash(datetime_dbconvert(unslash($datetime_added)))."', `amount`='".slash($amount)."',`account_id`='".slash($account_id)."',`details`='".slash($details)."'"." where id='".$id."'";
		doquery($sql,$dblink);
