...
if(isset($_POST["customer_payment_add"])){
	...
	if($err==""){
		$sql="INSERT INTO customer_payment (customer_id, machine_id, datetime_added, amount, discount, account_id, details, claim) VALUES ('".slash($customer_id)."','".slash($machine_id)."','".slash(datetime_dbconvert($datetime_added))."','".slash($amount)."','".slash($discount)."','".slash($account_id)."','".slash($details)."','".slash($claim)."')";
		doquery($sql,$dblink);
