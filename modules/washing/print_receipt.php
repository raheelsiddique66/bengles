...
if(isset($_GET["id"]) && !empty($_GET["id"])){
	...
	$incoming=dofetch(doquery("select * from washing where id='".slash($_GET["id"])."'", $dblink));
	?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div id="main">
    ...
    <div class="contentbox">
        ...
        <table class="table table-hover list">
            ...
            <?php
            $rs1 = doquery( "select *, group_concat(concat(size_id, 'x', quantity)) as sizes from washing_items where washing_id='".$incoming[ "id" ]."' group by color_id,design_id", $dblink );
            if(numrows($rs1)>0){
                $totals = [];
