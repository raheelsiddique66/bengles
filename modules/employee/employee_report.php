...
<div class="panel-body table-responsive">
	<table class="table table-hover list">
    	...
    	<tbody>
            ...
            <?php
            //$sql="select sum(calculated_salary) as amount from employee_salary a (select concat( 'Salary #', a.id) as transaction where employee_id = '".$employee[ "id" ]."' and date>='".date('Y-m-d',strtotime(date_dbconvert($date_from)))." 00:00:00' and date<'".date('Y-m-d',strtotime(date_dbconvert($date_to)))." 23:59:59' union select concat( 'Payment #', id) as transaction, -amount from employee_payment where employee_id = '".$employee[ "id" ]."' and date>='".date('Y-m-d',strtotime(date_dbconvert($date_from)))." 00:00:00' and date<'".date('Y-m-d',strtotime(date_dbconvert($date_to)))." 23:59:59') as transactions ";
