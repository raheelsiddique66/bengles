...
if(isset($_POST["employee_edit"])){
	...
	if($err==""){
		$sql="Update employees set `name`='".slash($name)."', `name_in_urdu`='".slash($name_in_urdu)."', `father_name`='".slash($father_name)."', `phone_number`='".slash($phone_number)."', `salary_type`='".slash($salary_type)."', `machine_id`='".slash($machine_id)."', `salary`='".slash($salary)."', `over_time_per_hour`='".slash($over_time_per_hour)."' where id='".$id."'";
		doquery($sql,$dblink);
