...
if(isset($_GET["action"]) && $_GET["action"]!=""){
	...
	if(empty($err)){
		if($bulk_action=="delete"){
			$i=0;
			while($i<count($id)){
				doquery("delete from employees where id='".$id[$i]."'",$dblink);
				$i++;
			}
		...
		if($bulk_action=="statuson"){
			$i=0;
			while($i<count($id)){
				doquery("update employees set status=1 where id='".$id[$i]."'",$dblink);
				$i++;
			}
		...
		if($bulk_action=="statusof"){
			$i=0;
			while($i<count($id)){
				doquery("update employees set status=0 where id='".$id[$i]."'",$dblink);
				$i++;
			}
