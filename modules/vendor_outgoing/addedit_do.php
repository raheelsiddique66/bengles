...
if(isset($_POST["action"])){
	...
	switch($_POST["action"]){
		...
		case "get_vendor_outgoing":
			$id = slash( $_POST[ "id" ] );
			$rs = doquery( "select a.*, b.name from vendor_outgoing a left join labour b on a.labour_id = b.id where a.id='".$id."'", $dblink );
			if( numrows( $rs ) > 0 ) {
				$r = dofetch( $rs );
				...
				$vendor_outgoing[ "vendor_outgoing_items" ] = $vendor_outgoing_items;
			}
			$response = $vendor_outgoing;
		break;
		case "save_vendor_outgoing":
			$err = array();
			$vendor_outgoing = json_decode( $_POST[ "vendor_outgoing" ] );
			if( empty( $vendor_outgoing->date ) || empty( $vendor_outgoing->vendor_id ) ) {
			...
			if( count( $err ) == 0 ) {
				if( !empty( $vendor_outgoing->id ) ) {
					doquery( "update vendor_outgoing set `date`='".slash(date_dbconvert($vendor_outgoing->date))."', `vendor_id`='".slash($vendor_outgoing->vendor_id)."', `labour_id`='".slash($vendor_outgoing->labour_id)."', `gatepass_id`='".slash($vendor_outgoing->gatepass_id)."' where id='".$vendor_outgoing->id."'", $dblink );
					$vendor_outgoing_id = $vendor_outgoing->id;
				}
				else {
					doquery( "insert into vendor_outgoing (date, vendor_id, labour_id, gatepass_id) VALUES ('".slash(date_dbconvert($vendor_outgoing->date))."', '".slash($vendor_outgoing->vendor_id)."', '".slash($vendor_outgoing->labour_id)."', '".slash($vendor_outgoing->gatepass_id)."')", $dblink );
