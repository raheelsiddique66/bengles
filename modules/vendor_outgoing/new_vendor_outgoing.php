...
if( isset( $_GET[ "date" ] ) ) {
    $_SESSION["vendor_outgoing_manage"]["new_vendor_outgoing"]["date"] = slash( $_GET[ "date" ] );
}
if(isset($_SESSION["vendor_outgoing_manage"]["new_vendor_outgoing"])){
    extract($_SESSION["vendor_outgoing_manage"]["new_vendor_outgoing"]);
}
else{
...
<div ng-app="vendoroutgoing" ng-controller="vendoroutgoingController" id="vendoroutgoingController">
    <div style="display:none">{{vendor_outgoing_id=<?php echo $id?>}}</div>
    <div class="page-header">
        <h1 class="title">{{get_action()}} Vendor Outgoing</h1>
        <ol class="breadcrumb">
            <li class="active">Manage Vendor Outgoing</li>
        </ol>
        <div class="right">
            <div class="btn-group" role="group" aria-label="..."> 
                <a href="vendor_outgoing_manage.php?tab=addedit" class="btn btn-light editproject">Add New Vendor Outgoing</a>
                <a href="vendor_outgoing_manage.php" class="btn btn-light editproject">Back to List</a>
            </div>
        </div>
    </div>
	<?php
        ...
        <div class="form-group">
            <div class="row">
                <div class="col-sm-2 control-label">
                    <label class="form-label" for="vendor_outgoing_date">Date *</label>
                </div>
                <div class="col-sm-10">
                    <input ng-model="date" class="form-control date-picker" ng-change="get_vendor_outgoing()" />
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            <div class="row">
                <div class="col-sm-1 control-label">
                    <label class="form-label">Vendor Outgoing Items <span class="manadatory">*</span></label>
                </div>
                <div class="col-sm-11">
                    ...
                    <table class="table table-hover list">
                            ...
                            <tbody>
                                <tr ng-repeat="vendor_outgoing in vendor_outgoings">
                                    ...
                                    <td>
                                        <table class="table table-hover list">
                                            ...
                                            <tr>
                                                <th>GatePass</th>
                                                <td style="position:relative">
                                                    <input ng-model="vendor_outgoing.gatepass_id" class="form-control" />
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                    <td>
                                        <table class="table table-hover list items-deliver">
                                            ...
                                            <tr ng-repeat="vendor_outgoing_item in vendor_outgoing.vendor_outgoing_items">
                                                <td>
                                                    <select title="Choose Option" ng-model="vendor_outgoing_item.machine_id" chosen>
                                                        <option value="">Select Machine</option>
                                                        <option ng-repeat="machine in machines" value="{{ machine.id }}">{{ machine.title }}</option>
                                                    </select>
                                                </td>
                                                <td>
                                                    <select title="Choose Option" ng-model="vendor_outgoing_item.design_id" chosen>
                                                        <option value="">Select Design</option>
                                                        <option ng-repeat="design in designs" value="{{ design.id }}">{{ design.title }}</option>
                                                    </select>
                                                </td>
                                                <td>
                                                    <select title="Choose Option" ng-model="vendor_outgoing_item.color_id" chosen>
                                                        <option value="">Select Color</option>
                                                        <option ng-repeat="color in colors" value="{{ color.id }}">{{ color.title }}</option>
                                                    </select>
                                                </td>
                                                <td class="text-right" ng-repeat="size in sizes"><input type="text" ng-model="vendor_outgoing_item.quantity[size.id]" /></td>
        ...
        <div class="form-group">
            <div class="row">
                <div class="col-sm-offset-2 col-sm-10">
                    ...
                    <button type="submit" ng-disabled="processing" class="btn btn-l <?php echo empty($id)?'btn-danger':"btn-default"?>" ng-click="save_vendor_outgoing()" title="Submit Record"><i class="fa fa-spin fa-gear" ng-show="processing"></i> SUBMIT</button>
                </div>
            </div>
        </div>
    </div>
