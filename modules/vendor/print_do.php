...
if( !empty( $id ) ){
	...
	<?php
    $sql="select concat( 'Vendor Delivery # ', a.gatepass_id, ' ', c.title, ' ', d.title) as transaction, date as datetime_added, unit_price * sum(quantity) as amount, sum(quantity) as quantity from vendor_delivery a left join vendor_delivery_items b on a.id = b.vendor_delivery_id left join color c on b.color_id = c.id left join design d on b.design_id = d.id where vendor_id = '".$vendor[ "id" ]."' and date>='".date('Y-m-d',strtotime(date_dbconvert($start_date)))." 00:00:00' and date<'".date('Y-m-d',strtotime(date_dbconvert($end_date)))." 23:59:59' group by a.id union select 'Payment', datetime_added as datetime_added, -amount, '' from vendor_payment where vendor_id = '".$vendor[ "id" ]."' and datetime_added>='".date('Y-m-d',strtotime(date_dbconvert($start_date)))." 00:00:00' and datetime_added<'".date('Y-m-d',strtotime(date_dbconvert($end_date)))." 23:59:59' order by datetime_added desc";
