...
if (($handle = fopen("data.csv", "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if(!empty($customer_id) && !empty($color_id) && !empty($design_id)) {
            if ($data[2] == "RECEVIED") {
                $table = "incoming";
            ...
            for ($i = 12; $i <= 16; $i++) {
                ...
                if (!empty($data[$i])) {
                    if ($data[2] == "RECEVIED") {
                        ...
                        doquery("insert into delivery_items(delivery_id, machine_id, color_id, size_id, design_id, quantity, extra, unit_price) values('" . $id . "', '0', '" . $color_id . "', '" . ($i - 11) . "', '" . $design_id . "', '" . $data[$i] . "', '" . (int)$data[7] . "', '" . (int)$data[8] . "')", $dblink);
                    }
                }
            }
        }
